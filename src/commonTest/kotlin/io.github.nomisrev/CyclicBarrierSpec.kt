...
class CyclicBarrierSpec :
  StringSpec({
    ...
    "should reset once full" {
      checkAll(Arb.constant(Unit)) {
        val barrier = DefaultCyclicBarrier(2)
    ...
    "race fiber cancel and barrier full" {
      checkAll(Arb.constant(Unit)) {
        ...
        when (raceN({ barrier.await() }, { job.cancelAndJoin() })) {
          // without the epoch check in CyclicBarrier, a late cancelation would increment the count
          // after the barrier has already reset, causing this code to never terminate (test times
          // out)
